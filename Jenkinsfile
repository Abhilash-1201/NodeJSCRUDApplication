        ...
        stage('Slack Notification') {
            steps {
                script {
                    ...
                    def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://3.16.143.158:9000/api/qualitygates/project_status?projectKey=nodejsproject" | jq -r .projectStatus.status').trim()
