        ...
        stage('Slack Notification') {
            steps {
                script {
                    ...
                    def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://18.224.3.77:9000/api/qualitygates/project_status?projectKey=maven" | jq -r .projectStatus.status').trim()
