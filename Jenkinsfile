        ...
        stage('Slack Notification') {
            steps {
                script {
                    ...
                    def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://3.144.216.208:9000/api/qualitygates/project_status?projectKey=nodejs" | jq -r .projectStatus.status').trim()
                    
                    if (qg == 'ERROR') {
                      slackSend color: '#FF0000', message: 'SonarQube Analysis failed. View the report at\n\nSonarQube Analysis Report : http://3.144.216.208:9000/dashboard?id=nodejs\n\nGuest Username: guest01\n\nGuest Password: guest01'
                  }
                  else if (qg == 'OK') {
